...
def f_inference_wrapper(args, pt_model, device, \
    ...
    # 假設 data_loader 是你的推理資料載入器
    with open(save_path, 'w') as f_out:
        for data_in, data_tar, data_info, idx_orig in tqdm(data_loader, desc="Inferencing"):
            data_in = data_in.to(device, dtype=nii_dconf.d_dtype)
            ...
            # 假設 data_gen 是模型輸出，例如 logits，轉成你要的 label
            predicted = torch.argmax(data_gen, dim=-1).cpu().numpy()

            for i, info in enumerate(data_info):
                # info 應該是 CON_D_0000000 這樣的檔名
                output_line = f"{info},{predicted[i]}"
